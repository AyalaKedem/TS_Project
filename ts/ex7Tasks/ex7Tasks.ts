...
import { taskManag } from "./taskManager.js";

const task_input = document.getElementById("task_input") as HTMLInputElement;
const task_div = document.getElementById("task_div") as HTMLDivElement;
const add_btn = document.getElementById("add_btn") as HTMLInputElement;
...
add_btn.addEventListener("click", () => {
  ...
  const task: Task = new Task(task_input.value);
  document.querySelectorAll('p').forEach(p => p.remove())
  taskManag.add(task);
  createTask(task);
  task_input.value = '';
...
function createTask(task: Task) {
  ...
  // correct_sign.id = task.timeStampId;
  ...
  // cross_sign.id = task.timeStampId;
  ...
  // edit_sign.id = task.timeStampId;
  edit_sign.addEventListener("click", () => {
    task.title = input.value; //
    input.disabled = !input.disabled // input.disabled = false;
    // input.disabled ? input.classList.remove('edit-task') : input.classList.add('edit-task');
    taskManag.edit(task);
  });
